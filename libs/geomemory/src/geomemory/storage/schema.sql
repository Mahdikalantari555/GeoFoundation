...
CREATE INDEX IF NOT EXISTS idx_proposal_type ON knowledge_change_proposal(proposal_type);

CREATE TABLE IF NOT EXISTS provenance_chain (
    id              TEXT PRIMARY KEY,
    candidate_id    TEXT NOT NULL REFERENCES candidate_memory(id) ON DELETE CASCADE,
    feedback_id     TEXT NOT NULL,
    created_at      TEXT NOT NULL DEFAULT (datetime('now'))
);
CREATE INDEX IF NOT EXISTS idx_provenance_candidate ON provenance_chain(candidate_id);
